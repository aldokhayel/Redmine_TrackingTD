...
Redmine::MenuManager.map :application_menu do |menu|
  ...
  menu.push :issues,   {:controller => 'issues', :action => 'index'},
            :if => Proc.new {
                     User.current.allowed_to?(:view_issues, nil, :global => false) &&
  ...
  menu.push :time_entries, {:controller => 'timelog', :action => 'index'},
            :if => Proc.new {
                     User.current.allowed_to?(:view_time_entries, nil, :global => false) &&
  ...
  menu.push :gantt, { :controller => 'gantts', :action => 'show' },
            :caption => :label_gantt,
            :if => Proc.new {
                     User.current.allowed_to?(:view_gantt, nil, :global => false) &&
  ...
  menu.push :calendar, { :controller => 'calendars', :action => 'show' },
            ...
            :if => Proc.new {
                     User.current.allowed_to?(:view_calendar, nil, :global => false) &&
                       EnabledModule.exists?(:project => Project.visible, :name => :calendar)
                   }
  menu.push :news, {:controller => 'news', :action => 'index'},
            :if => Proc.new {
                     User.current.allowed_to?(:view_news, nil, :global => false) &&
